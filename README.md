...
1. Функция подсчета коэффициента предметности (Pr) `subjectivity_coefficient(self)` – отношение суммы существительных и местоимений к сумме прилагательных и глаголов.
2. Функция подсчета коэффициента качественности (Qu) `quality_coefficient(self)` – отношение суммы прилагательных и наречий к сумме глаголов и существительных.
3. Функция подсчета коэффициента активности (Ac) `activity_coefficient(self)` – отношение суммы глаголов и глагольных форм к количеству слов в тексте.
4. Функция подсчета коэффициента динамизма (Din) `dynamism_coefficient(self)` –  отношение суммы глаголов и глагольных форм к сумме существительных, прилагательных и местоимений.
5. Функция подсчета коэффициента связности текста (Con) `cohesion_coefficient(self)` – отношение суммы предлогов и союзов к числу предложений.

## Поиск ключевых слов

В качестве референтного корпуса использовался корпус [OpenCorpora](https://www.opencorpora.org/)

ключевые слова с помощью разных метрик (Log-Likelihood, t-score, Dice, MI, Frequency)
## Поиск коллокаций с ключевыми словами
